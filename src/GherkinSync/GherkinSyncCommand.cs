...
namespace GherkinSync
{
    ...
    internal sealed class GherkinSyncCommand
    {
        ...
        private GherkinSyncCommand(AsyncPackage package, OleMenuCommandService commandService)
        {
            ...
            _parser = new GherkinParser(_options);
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Usage", "VSTHRD002:Avoid problematic synchronous waits", Justification = "The method Execute needs to run in the main thread.")]
        private void Execute(object sender, EventArgs e)
        {
            ...
            if (currentFilePath.EndsWith(".feature"))
            {
                ...
                var featureBackgroundSteps = featureBackground != default ? _parser.StepsToList(featureBackground.Steps) : new List<string>();

                var testCasesList = _parser.ConvertToTestCases(featureScenarios, featureBackgroundSteps, feature.Name, feature.Description);
                ...
                foreach (var featureRule in featureRules)
                {
                    ...
                    var ruleBackgroundSteps = featureBackgroundSteps.Count > 0 ?
                        ...
                        _parser.StepsToList(featureBackground.Steps) :
                        new List<string>();

                    var ruleScenarios = featureRule.Children.Where(c => c.GetType() == typeof(Scenario)).Cast<Scenario>();

                    testCasesList.AddRange(_parser.ConvertToTestCases(featureScenarios, featureBackgroundSteps, feature.Name, feature.Description, featureRule.Name, featureRule.Description));
                }

                foreach (var testCase in testCasesList)
                {
                    testCase.TestCaseId = CreateOrUpdateTestCaseAsync(testCase).Result;
                }
            }
        }

        public async Task<int> CreateOrUpdateTestCaseAsync(TestCase testCase)
        {
            using (var connection = new VssConnection(new Uri(_options.AzureDevopsBaseUrl), new VssBasicCredential("", _options.PatToken)))
            {
                try
                {
                    ...
                    workItem.Fields.AddOrUpdate("Description", _options.DescriptionTemplate
                    ...
                    {
                        workItem = await workItemClient.CreateWorkItemAsync(patchDocument, _options.ProjectName, "Test Case");
                    }
                }
                catch (Exception ex)
                {
